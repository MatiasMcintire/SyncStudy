...
const App = {
  ...
  bindEvents() {
    ...
    if (groupSelector) {
      groupSelector.addEventListener('change', (e) => {
        const group = Storage.setActiveGroup(e.target.value);
        if (!group) return;
    ...
    // Filtros y visibilidad de grupos
    const sidebarGroupSearch = $('#sidebarGroupSearch');
    if (sidebarGroupSearch) {
      sidebarGroupSearch.addEventListener('input', () => Views.renderSidebarGroups());
    }

    const sidebarToggleHiddenGroups = $('#sidebarToggleHiddenGroups');
    if (sidebarToggleHiddenGroups) {
      sidebarToggleHiddenGroups.addEventListener('click', () => {
        sidebarToggleHiddenGroups.classList.toggle('is-active');
        Views.renderSidebarGroups();
        refreshIcons();
      });
    }
    ...
    if (groupSearchInput) {
      groupSearchInput.addEventListener('input', () => Views.renderGroupManager());
    }

    const showHiddenGroups = $('#showHiddenGroups');
    if (showHiddenGroups) {
      showHiddenGroups.addEventListener('change', () => Views.renderGroupManager());
    }
  ...
  handleCreateGroup() {
    ...
    if (!name) {
      showToast('Escribe un nombre para el grupo', 'alert-circle');
      return;
    }

    Storage.createGroup(name);
    input.value = '';
    showToast('Grupo creado', 'folder-plus');
    Views.renderAll();
  },
  ...
  handleAddMember() {
    ...
    if (!name) {
      showToast('Escribe el nombre del integrante', 'alert-circle');
      return;
    }

    Storage.addMember({ name });
    input.value = '';
    showToast('Integrante agregado', 'user-plus');
    Views.renderAll();
  },
  ...
  calendarNav(direction) {
    if (calendarState.mode === 'week') {
      calendarState.anchorDate = addDays(calendarState.anchorDate, direction * 14);
    } else {
